...
namespace SIT.Launcher
{
    ...
    public partial class MainWindow : MetroWindow, ILogger
    {
        public MainWindow()
        {
            ...
            this.Title = "SIT 启动器 - " + App.ProductVersion.ToString();
        ...
        private async Task UpdateInstallFromOfficial()
        {
            ...
            MessageBoxResult messageBoxResult = MessageBox.Show("检测到有更新,是否现在更新? (警告,这可能会使SIT模组报错!)", "EFT Update Detected", MessageBoxButton.YesNo);
        ...
        private async void NewInstallFromOfficial()
        {
            ...
            {
                if (MessageBox.Show("未找到离线版塔科夫. 是否现在安装?", "Install", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                {
                    ...
                    folderBrowserDialogOffline.Title = "选择新的离线塔科夫安装文件夹";
                    if (folderBrowserDialogOffline.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        {
                            MessageBox.Show("您无法将离线版安装到您的官方文件夹中!", "Install");
                            NewInstallFromOfficial();
                            return;
                        }
        ...
        private string LoginToServer()
        {
            ...
            {
                MessageBox.Show("未填写服务器地址");
                return null;
            }

            if (ServerAddress.EndsWith("/"))
            {
                MessageBox.Show("服务器地址不正确，结尾不应该有/!");
                return null;
            }
            ...
            try
            {
                ...
                {
                    var messageBoxResult = MessageBox.Show("找不到你的帐户，是否为您注册一个新帐户？", "Account", MessageBoxButton.YesNo);
                    if (messageBoxResult == MessageBoxResult.Yes)
                    {
                        // Register
            ...
            {
                MessageBox.Show(webEx.Message, "无法与服务器通信");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "无法与服务器通信");
            }
            return null;
        }
        ...
        private async void btnLaunchGame_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var messageBoxResult = MessageBox.Show("出了问题。也许服务器还没有启动？检查日志.", "Account");
                return;
            }
            ...
            {
                var messageBoxResult = MessageBox.Show("用户名/电子邮件已被使用，请更换.", "Account");
            }
        ...
        private async Task DownloadInstallAndStartGame(string sessionId)
        {
            ...
            {
                MessageBox.Show("你的游戏还未离线化,也没有安装可以离线游戏的客户端MOD。请安装SIT或手动卸载.");
        ...
        private void CollapseAll()
        {
            ...
            gridKook.Visibility = Visibility.Collapsed;
        ...
        private void btnKook_Click(object sender, RoutedEventArgs e)
        {
            CollapseAll();
            gridKook.Visibility = Visibility.Visible;
        }
